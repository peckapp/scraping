...
# a class that parses rss feeds into the appropriate format for the database

# sill not run without rails app, here for reference since this is no longer the used format for scraping

class RssParser
  def self.parse
    puts 'in scrape method'
    Tasks::RssPage.all.each do |page|
